...
namespace WebBank.Pages
{
    public class DepositContractModel(MySQLContext context, IDepositService depositService, ITimeService timeService) : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            {
                await _depositService.Create(client, program, Convert.ToInt32(StartPayment * 100), sysTime, ContractName);
            }
            else
            {
                await _depositService.Create(client, program, Convert.ToInt32(StartPayment * 100), sysTime);
            }
